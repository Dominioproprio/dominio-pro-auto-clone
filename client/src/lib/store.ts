...
function toService(r: any): Service {
  return { id: r.id, name: r.name, description: r.description ?? null, durationMinutes: r.duration_minutes ?? 60, price: Number(r.price ?? 0), material_cost_percent: Number(r.material_cost_percent ?? 0), color: r.color ?? "#ec4899", active: r.active ?? true, createdAt: r.created_at };
}
